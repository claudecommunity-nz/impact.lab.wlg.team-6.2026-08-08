...
## The commercial track: Kitea

`kitea/` is the same two-way loop rebuilt the day after the event as the
product a council could adopt, under a working name that states the promise:
*kitea*, te reo for "seen". The commercial case, the inclusion stance, the
NZ Privacy Act mapping and the honest pilot blockers are in
[COMMERCIAL.md](COMMERCIAL.md).

```bash
python3 -m kitea          # residents http://127.0.0.1:8146/  ops /ops
KITEA_OPS_KEY=<key> python3 scripts/seed_demo.py http://127.0.0.1:8146
```

Same zero-dependency rule as the event build (stdlib + SQLite), different
trade-offs now that there is no demo clock:

- **Live status push.** The reporter's page updates over SSE the moment a
  duty officer taps a status; reference codes are 8-character credentials
  with throttled lookups instead of demo-length codes.
- **All the live feeds, one operating picture.** GWRC river and rain gauges
  (with trend and freshness), Waka Kotahi delays and cameras, GeoNet felt
  quakes, NEMA electricity outages, MetService CAP warnings drawn as
  polygons, WREMO hubs. Every feed card shows its age; stale is visible,
  never hidden.
- **Interactive basemap.** MapLibre and the fonts are vendored into the
  repo, keeping the no-CDN rule while allowing pan and zoom; tiles are the
  one remaining network dependency, noted as a pilot blocker.
- **Hazard context on every report** using this repo's `enrichment/` fixes,
  plus tsunami `Col_Code`/`Evac_Zone`, `liquefaction-regional` and
  active-fault field corrections verified live on 2026-08-10.

It already closes several items from the list below: photo upload with magic
byte checks, reporter notification (SSE), and an ops queue with one-tap
statuses. Both builds stay in the repo on purpose: the event build is the
submission and demos offline; kitea is where the council conversation goes
next. `python3 -m unittest discover tests` runs both suites.
